...
from gpl.datasets.spmotif import to_undirected_customized
...
class Mutag(InMemoryDataset):
    def __init__(self, root, device=None):
        ...
        # self.data = self.data.to(self.device) # set dataset location
        # self.slices = self.slices.to(self.slices)
    ...
    def process(self):
        ...
        for i in range(original_labels.shape[0]):
            ...
            edge_index, edge_attr, edge_label = to_undirected_customized(edge_index=edge_index, edge_attr=None, edge_label=edge_label, reduce='mean')

            data = Data(x=x, y=y, edge_index=edge_index, node_label=node_label, edge_label=edge_label, node_type=torch.tensor(node_type_lists[i]))
            
            assert data.is_undirected(), "应该是undirected。"
